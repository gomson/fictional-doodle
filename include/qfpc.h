  ...
  int key_d)
{
  ...
  const float leftAxis[3] = { -1.f,  0.f,  0.f };
  const float upAxis[3]   = {  0.f,  1.f,  0.f };
  const float downAxis[3] = {  0.f, -1.f,  0.f };
  const float farAxis[3]  = {  0.f,  0.f, -1.f };
  qfpcQAxisAngle(y_quat, leftAxis, (y_delta * look_mult) * QFPC_TO_RAD);
  qfpcQAxisAngle(x_quat, downAxis, (x_delta * look_mult) * QFPC_TO_RAD);
  ...
  qfpcV3Rotq(sd_vec3, leftAxis, cam_quat);
  qfpcV3Rotq(up_vec3, upAxis, cam_quat);
  qfpcV3Rotq(fw_vec3, farAxis, cam_quat);
